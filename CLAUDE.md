...
- **Guardrails are instructions, not guarantees.** Never imply that a skill's `Do not` list or quality-control checks are enforceable controls. They are prompt-level instructions an AI may not follow perfectly. Human review is what catches an unsafe output, but review is a control only where something requires it. What enforces is design: access never granted, outputs that are drafts by construction, and a gate the workflow cannot complete without. See [docs/action-authority-model.md](docs/action-authority-model.md).
